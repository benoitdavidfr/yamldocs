...
/*PhpDoc:
name: loadtopo.php
title: loadtopo.php - test d'un chargement des données dans une topology PostGis
doc: |
  Test du chargement de Rpicom comme base géo dans une topology PostGis

  Lors du chargement des communes dans PostGis, génération d'erreurs de topologie (6 erreurs).
  Ces erreurs sont dues à la simplification que j'ai effectuée en arrondissant les coord. à 1 mètre.
  Le jeu initial IGN comporte des détails de l'ordre d'1 mètre qui génèrent ces erreurs topologiques.
  La solution pourrait être de ne pas effectuer cette simplification avant le chargement dans PostGis.
  Solution abandonnée car le module topology est trop contraignant.
  De plus, la fonction ST_SimplifyPreserveTopology() n'utilise pas cette topologie.

  schémaDeDonnées: |
    -- aegeofla_topo contient le graphe topologique
    -- génère une erreur avec 0.0001 (10 m), pas avec 0.000001 (10 cm), ni avec 0.00001 (1 m).
    SELECT topology.CreateTopology('aegeofla_topo', 4326, 0.00001);

    create type admin_statut AS ENUM ('cSimple', 'cDéléguée', 'cAssociée', 'ardtMun');
    -- table des entités admin. (communes ou entités rattachées) versionnées définies par leur code INSEE, date de début et topologie
    drop table public.eadminv cascade;
    create table public.eadminv(
      gid serial,
      cinsee char(5) not null,
      statut admin_statut not null,
      crat char(5), -- pour une entité rattachée code INSEE de la c. de rattachement, null sinon
      debut date not null, -- premier jour, 1/1/1943 par défaut
      fin date, -- lendemain du dernier jour, null si encore valide 
      -- bbox box2d, -- le rectangle englobant
      geom geometry(MULTIPOLYGON), -- la géométrie
      primary key (cinsee, debut)
    );
    select topology.AddTopoGeometryColumn('aegeofla_topo', 'public', 'eadminv', 'topo', 'MULTIPOLYGON');
    -> 1
  
    CREATE INDEX eadminbbox ON public.eadminv USING GIST (geom); 
  
  ST_SimplifyPreserveTopology(geometry geomA, float tolerance);
  
  -- inadapté - ST_SimplifyPreserveTopology() doit travailler sur la totalité de la géométrie et pas objet par objet
  drop table public.seadminv;
  create table public.seadminv as
  select ST_SimplifyPreserveTopology(geom, 0.01) geom from public.eadminv;
  CREATE INDEX seadminind ON public.seadminv USING GIST (geom); 
